...
//Fin	
		...
		consultarTiposSolicitudPorAreaDelFuncionario('tableSolsXAtender', <%= Areas.ATENCION_VICTIMAS.parseLong() %>);
		...
		*/
		function cargaTodasLasSolicitudes(){

			if(primeraConsulta == true){
				
				jQuery("#gridSolsXAtndr").jqGrid({ 
					url:'<%= request.getContextPath()%>/consultaSolsXAtnderUAVD.do?tipoSoliciutd='+idsTiposSolicitudes+'&idArea=0&estatus=<%=EstatusSolicitud.ABIERTA.getValorId()%>,<%=EstatusSolicitud.EN_PROCESO.getValorId()%>', 
					datatype: "xml", 
				...
				jQuery("#gridSolsXAtndr").jqGrid('setGridParam', {url:'<%= request.getContextPath()%>/consultaSolsXAtnderUAVD.do?tipoSoliciutd='+idsTiposSolicitudes+'&idArea=0&estatus=<%=EstatusSolicitud.ABIERTA.getValorId()%>,<%=EstatusSolicitud.EN_PROCESO.getValorId()%>',datatype: "xml" });
			...
			$.ajax({
				...
				success: function(xml){
					$(xml).find('ValorDTO').each(function(){
						...
						trTabla = trTabla + '<span class="file"><a onclick="cargaGridSolsXAtndr('+$(this).find("idCampo").text()+','+idArea+')">'+$(this).find("valor").text()+'</a></span>';
						trTabla = trTabla + '</li>';
...
<div class="ui-layout-west">
	...
	<div class="content">
		<div id="accordionmenuprincipal">
			<h3><a href="#"><span id="cargaTodasSols">Solicitudes por Atender</span></a></h3>
			<div>
			...
			<TABLE border=0 cellSpacing=0 cellPadding=0 width="100%"  height="100%">
				  <TBODY style="background: #fff;">
					  <TR>
					    ...
					    <TD width=301 align=left valign="middle" style="color:#51a651;">COORDINADOR  DE UNIDAD DE VICTIMAS DEL DELITO</TD>
