...
//**********************************************************FUNCIONALIDAD DE LA CEJA INDIVIDULIZACION*************************************************************\
	...
	*/		
	function cargaGridFiscal(){
		...
		//$("#t_gridFiscal").append("<input type='button' class='btn_mediano' id='butamp' value='Aviso Coordinador Amp' style='height:20px;font-size:-3'/>"+" "+"<input type='button' class='btn_modificar' id='butAgreAmp' value='Agregar Amp' style='height:20px;font-size:-3'/>"+" "+"<input type='button' class='btn_modificar' id='butNotificacionFiscal' value='Notificaciones' style='height:20px;font-size:-3' disable='disable'/>");
		$("#t_gridFiscal").append("<input type='button' class='btn_Generico' id='butAgreAmp' value='Agregar Amp' style='height:20px;font-size:-3'/>");
		$("#butamp","#t_gridFiscal").click(function(){
				
			//idWindowIngresarDenunciante++;
			//$.newWindow({id:"iframewindowIngresarDenunciante" + idWindowIngresarDenunciante, statusBar: true, posx:150,posy:20,width:1040,height:570,title:"Ingresar Fiscal", type:"iframe"});
		    //$.updateWindowContent("iframewindowIngresarDenunciante" + idWindowIngresarDenunciante,'<iframe src="<%= request.getContextPath() %>/IngresarDenunciante.do?numeroExpediente='+numExpediente +'&calidadInv=FISCAL" width="1040" height="570" />');		
		});
	...
	*/		
	function cargaGridDefensor(){
		...
		$("#t_gridDefensor").append("<input type='button' class='btn_Generico' id='butADefensor' value='Agregar Defensor' style='height:20px;font-size:-3'/>"+" "+"<input type='button' class='btn_Generico' id='butSDefensor' value='Solicitar Defensor' style='height:20px;font-size:-3'/>");
	...
	*/
	function cargaGridPerito(){
		...
		$("#t_gridPerito").append("<input type='button' class='btn_Generico' value='Agregar Perito' style='height:20px;font-size:-3'/>");
	...
	*/
	function cargaGridPolicia(){
		...
		$("#t_gridPolicia").append("<input type='button' class='btn_Generico' value='Agregar elemento de la policía ministerial' style='height:20px;font-size:-3'/>");
...
//*****************************************************FUNCIONALIDAD DE LA CEJA RESOLUTIVOS DE AUDIENCIA**********************************************************\
	...
	*/
	function agregarRegistrarResolutivo(){
		...
		width: 350, height: 300, maxWidth: 350,
		buttons:{
			"Enviar":function() {			  		
...
//**********************************************************FUNCIONALIDAD DE LA CEJA PROGRAMAR AUDIENCIA**********************************************************\ 	
				...
				if(lstImputados.length > 0){
					...
					$.ajax({
						...
						success: function(xml){
								...
								if(mensaje == "fail"){
									alertDinamico("La audiencia ya fue programada anteriormente, ó por algún otro usuario");
									/*
	...
	*/
	function cargaGridTestigo1(){
		...
		if(banderaGridTestigo1 == true){
			...
			$("#t_gridTestigo1").append("<input type='button' class='btn_Generico' value='Agregar Testigo' style='height:20px;font-size:-3'/>");
	...
	*/
	function cargaGridPerito1(){
		...
		jQuery("#gridPerito1").jqGrid({
			...
			$("#t_gridPerito1").append("<input type='button' class='btn_Generico' value='Agregar Perito' style='height:20px;font-size:-3'/>");
	...
	*/
	function cargaGridPolicia1(){
		...
		$("#t_gridPolicia1").append("<input type='button' class='btn_Generico' value='Agregar elemento de la policía ministerial' style='height:20px;font-size:-3'/>");
	...
	*/		
	function cargaGridDefensor1(){
		...
		$("#t_gridDefensor1").append("<input type='button' class='btn_Generico' id='butADefensor1' value='Agregar Defensor' style='height:20px;font-size:-3'/>"+" "+"<input type='button' class='btn_Generico' id='butSDefensor1' value='Solicitar Defensor' style='height:20px;font-size:-3'/>");
...
//**********************************************************************FUNCIONALIDAD DESCONOCIDA****************************************************************
	...
	*/
	...
	function guardarRR(){alertDinamico('Recurso y resolución guardados correctamente');}
	
	//********************************************************************************************************************************************************************/
		
	function consultarAudienciaJAVS(){		
		$.ajax({
			type: 'POST',
			url:'<%= request.getContextPath()%>/consultarAudienciaJavs.do?idAudiencia='+idAudiencia+'',
			data:'',
			dataType: 'xml',
			async: false,
			success: function(xml){
				...
				var S_Mensaje=mensajeEstadoJAVS(respuesta);
				if(S_Mensaje!=""){
					alertDinamico(S_Mensaje);
				}
				else{
					alertDinamico("Fallo al conectar con el Web Service");
				}
	...
	function mensajeEstadoJAVSEliminacion(idEvento){
        ...
        case "<%=ConstantesGenerales.NO_ES_JAVS%>":
            S_Mensaje = "La audiencia no se llevo a cabo en Sala JAVS";
            break;                      
        }		
        return S_Mensaje;
	}
   ...
   function cierraVentanaRegistrarAmparo(){
	...
	function ismaxlength(obj,size){
		...
		//if (obj.getAttribute && obj.value.length>size)
	...
	<div id="tabsDetalleAudiencias-4">
		<table width="1060" align="center" height="370" class="back_obj" border="0" cellpadding="0" cellspacing="5">
			...
			<tr>
				<td>&nbsp;</td>
			    <td align="center">
			    	<input class="btn_Generico" type="button" id="agregarResolutivoBoton" value="Agregar Registro" />
			    </td>
			    <td>
			    	<input type="button" class="btn_Generico" id="eliminarResolutivoBoton" value="Eliminar Registro" />
			    </td>
				<td>
					<input type="button" class="btn_Generico" id="btnUVAD" value="Notas de JAVS" onclick="consultarAudienciaJAVS();"/>
				</td>
				<td>
					<input type="button" class="btn_Generico" id="btnJAVS" value="Eliminar agenda de JAVS" onclick="eliminarAudienciaJAVS();"/>
				</td>				
				<td width="15%" align="left" valign="top">&nbsp;</td>
			</tr>
	  		<tr>	
		...
		<table width="1060" align="center" height="354" class="back_hechos" border="0" cellpadding="0" cellspacing="0">
             ...
             <tr>
			  <td width="18%" align="right"><ul class="botonLeyes">
						
					</ul></td>
	  		</tr>	
			<tr valign="middle">
				<td width="5%" height="40" align="right"><ul class="botonLeyes">
					<div>
						<li id="consPolitica" onclick="abreCodigosLeyes(2070)" style="cursor: pointer;"><span></span>Constitución Política</li>
					</div>
				</ul></td >
			...
			<tr>		
				<td width="5%" height="39" align="right" class="linea_derecha_gris"><ul class="botonLeyes">
					<div>
							<li id="leyesGenerales" onClick="abreCodigosLeyes(2071)" style="cursor: pointer;"><span>Leyes Generales</span></li>
					</div>
				</ul></td>
			</tr>
			<tr>
				...
				<td width="5%" height="40" align="right" class="linea_derecha_gris"><ul class="botonLeyes">
					<div>
						  <li id="tratados" onClick="abreCodigosLeyes(2074)" style="cursor: pointer;">Tratados Internacionales</li>
					</div>
				</ul></td>
			</tr>
			<tr>
				<td  width="5%" height="40" align="right" class="linea_derecha_gris"><ul class="botonLeyes">
					<div>
							<li id="codigos" onClick="abreCodigosLeyes(2072)" style="cursor: pointer;"><span></span>Códigos</li>
					</div>
				</ul></td>
	  		</tr>
			<tr>
			  <td width="5%" height="40" align="right" class="linea_derecha_gris"><ul class="botonLeyes">
						<div>
							<li id="acuerdos" onClick="abreCodigosLeyes(2075)" style="cursor: pointer;"><span></span>Acuerdos</li>
						</div>
					</ul></td>
	  		</tr>
			<tr>
			  <td width="18%" height="40" align="right" class="linea_derecha_gris"><ul class="botonLeyes">
						<div>
							<li id="circulares" onClick="abreCodigosLeyes(2076)" style="cursor: pointer;" ><span></span>Circulares</li>
						</div>
					</ul></td>
	  		</tr>
			<tr>
			  <td  width="18%" height="39" align="right" class="linea_derecha_gris"><ul class="botonLeyes">
						<div>
							<li id="manuales" onClick="abreCodigosLeyes(2073)" style="cursor: pointer;"><span></span>Manuales</li>
						</div>
					</ul></td>
	  		</tr>
			<tr>
			  <td width="18%" align="right"><ul class="botonLeyes">
						<div>
							<li id="instructivos" onClick="abreCodigosLeyes(2077)" style="cursor: pointer;"><span>Instructivos</span></li>
						</div>
					</ul></td>
	  		</tr>	
            <tr>
	...
	<div id="tabsDetalleAudiencias-7">
		...
		<table width="720" cellspacing="0" cellpadding="0">
			...
			<tr>
				...
				<td>
					<input type="button" class="btn_Generico" value="Finalizar Audiencia" onclick="finalizarAudiencia();">
				</td>
			</tr>
			<tr>
				<td>&nbsp;</td>
